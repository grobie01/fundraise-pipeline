...
import { getAttioListEntries, mapAttioEntryToInvestor } from '@/lib/attio';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { attioListId, listName } = body;
    ...
    // Map Attio entries to our format
    const investors = entries.map(entry => mapAttioEntryToInvestor(entry));
